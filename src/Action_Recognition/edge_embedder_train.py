...
from src.Synthetic.models.dataset_build_utils import build_video_edgeembedder_dataset
    ...
    for data_i in range(len(dataset_train)):
        ...
        for index in range(len(dataset_train[data_i][1])):
            ...
            input = torch.cat((torch.cat((out_formula, out_true[-1])).flatten().unsqueeze(0), \
                                     torch.cat((out_formula, out_false[-1])).flatten().unsqueeze(0)))
        ...
        if data_i % 128 == 0 and data_i != 0:
            ...
            out_true = []
            count=0
    return np.mean(loss_list_cross_entropy), np.mean(acc_list)
...
def get_dataset(dataset_save_path):
    ...
    dataset_train, dataset_test = build_video_edgeembedder_dataset(device, args.dataset_root,
                                                             args.dataset_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_name", type=str, default="/edge_embedder_dataset/")
    parser.add_argument("--margin", type=float, default=2)
    parser.add_argument("--model_save_path", type=str, default="./saved_models/Action_Recognition/edge_embedder/")
    parser.add_argument("--rebuild_dataset", default=False, action="store_true")
    parser.add_argument("--device_id", type=int, default=0)
    parser.add_argument("--load_saved_edge_embedder", type=bool, default=False, help="load saved edge embedder")
    parser.add_argument("--saved_edge_embedder_path", type=str, default="./saved_models/Action_Recognition/edge_embedder/edge_embedder_latest.pt")
    args = parser.parse_args()
    dataset_save_path = args.dataset_root + args.dataset_name + "saved_dataset.pt"
    args.cuda = not args.no_cuda and torch.cuda.is_available()
    device = torch.device('cuda:' + str(args.device_id) if args.cuda else 'cpu')

    if args.load_saved_edge_embedder:
        edge_embedder = torch.load(args.saved_edge_embedder_path, map_location=device)
    else:
        edge_embedder = Edge_Embedder(INPUT_DIMENSION = 200, HIDDEN_DIMENSION=256, OUTPUT_DIMENSION=100)
    ...
    for epoch in range(50):
        print ('Epoch', epoch)
        torch.save(edge_embedder, args.model_save_path +'edge_embedder'+ "_latest.pt")
        torch.save(edge_embedder, args.model_save_path + 'edge_embedder' + "_"+str(epoch)+".pt")
        for i in range(10):
            loss=train_embedder()
        ...
        for i in range(10):
            loss, acc=train_mlp()
            print("train mlp at epoch {}: loss is {}; acc {}".format(i, loss, acc))
            eval()
