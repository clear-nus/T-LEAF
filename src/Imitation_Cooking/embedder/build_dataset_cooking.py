...
from src.Synthetic.models.Util import read_pk_file, finite2aut, aut2graph, write_strlist
...
if __name__ =="__main__":
    ...
    parser.add_argument("--dataset_root", type=str, default="./datasets/Imitation_Cooking/")
    parser.add_argument("--dataset_edge_name", type=str, default="/edge_embedder_dataset/")
    parser.add_argument("--dataset_meta_name", type=str, default="/meta_embedder_dataset/")
    ...
    while index < args.num_formula:
        ings, prop_dict, next_relation, ltl_next_relation, feasible_relation, ltl_feasible_relation = extract_relation()
        size = len(prop_dict)
        if size == 0:
        ...
        name = str(index) + "_"
        name += "_".join([ingreds_name[i] for i in ings])
        numerical_video_dataset(name, word_embedding, graph_ltl_video, true_assigns, false_assigns)
        numerical_edge_dataset(prop_dict, edge_dataset_video, name)
        index += 1
